...
```{r}
# saveRDS(umweltzonen, "data/umweltbundesamt/umweltonen.rds")
# readRDS("data/umweltbundesamt/umweltonen.rds")
```
...
nuts.sf <- nuts.sf %>%
  mutate(
    ...
    with_uar_50km = ifelse(
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, get_buffer_km(uar_sf, 50))$NUTS_ID %>%
           unique()),
      TRUE, FALSE),
    with_uar_100km = ifelse(
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, get_buffer_km(uar_sf, 100))$NUTS_ID %>%
           unique()),
      TRUE, FALSE),
      ...
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, gzones_sf)$NUTS_ID %>% 
           unique()),
      TRUE, FALSE),
    with_gzones_50km = ifelse(
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf,  get_buffer_km(gzones_sf, 50))$NUTS_ID %>%
           unique()),
      TRUE, FALSE),
    with_gzones_100km = ifelse(
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, get_buffer_km(gzones_sf, 100))$NUTS_ID %>%
           unique()),
      TRUE, FALSE),
      ...
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, umweltzonen)$NUTS_ID %>% 
           unique()), 
      TRUE, FALSE),
    with_umweltzonen_50km = ifelse(
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, get_buffer_km(umweltzonen, 50))$NUTS_ID %>%
           unique()), 
      TRUE, FALSE),
    with_umweltzonen_100km = ifelse(
      NUTS_ID %in% 
        (st_intersection(
          nuts.sf, get_buffer_km(umweltzonen, 100))$NUTS_ID %>%
           unique()),
      TRUE, FALSE)
  )
...
# + include locations of cities in umweltbundesamt
...
nuts.sf <- nuts.sf %>%
  mutate(
    ...
    contr_restr_50km = case_when(
      with_umweltzonen_50km ~ FALSE,
      with_gzones_50km ~ FALSE,
      with_uar_50km ~ FALSE,
      TRUE ~ TRUE # if none of the past conditions, it's a control
    ),
    contr_restr_100km = case_when(
      with_umweltzonen_100km ~ FALSE,
      with_gzones_100km ~ FALSE,
      with_uar_100km ~ FALSE,
      TRUE ~ TRUE # if none of the past conditions, it's a control
    )
  )
nuts.sf %>% names()
...
htmlwidgets::saveWidget(treated_and_controls,
                        file="map_tr_and_controls_.html")
...
nuts.sf.treated <- nuts.sf_treated %>%
  mutate(more_one_nuts = duplicated(NUTS_ID)) %>%
  dplyr::group_by(NUTS_ID) %>%
  dplyr::mutate(more_one_nuts = sum(more_one_nuts)) %>%
  ungroup()

# List og NUTS regions that have only one LEZ: 
nuts.sf.treated %>%
  filter(!is.na(LEZ),
         more_one_nuts == 0)

nuts.sf.treated <- temp %>%
