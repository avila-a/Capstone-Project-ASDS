...
dta_synth <- main_dta %>% as.data.frame() %>%
  ...
  # # Optional? restrict years ro 2001-2017,
  filter(time >= 2000 & time <= 2017)  # Not all of them have data for 2008
...
control_vars <- c("urb_index", "gdp_eur_hab", "wages_per_worker")
outcome <- "unempR_T"
remove_countries <-  c("ES", "EL", "PT", "PL")
...
dta_synth <- balanced(data = dta_synth, TIME = "time", ID = "geo", 
                      VARS = c(control_vars, 
                               outcome))
...
dataprep.out <-
  dataprep(
           foo = dta_synth,
           predictors    = c(outcome, control_vars),
           dependent     = outcome,
           ...
           treatment.identifier = "DE92", # 7 is DE30 (Berlin), 8 is Bremen
...
synth.tables[[3]] %>% as.data.frame() %>% arrange(desc(w.weights) )
```
