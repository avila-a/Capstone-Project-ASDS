...
NEW: Create a control for Scrappage program (months the scrappage program was active)

> NOTE! I WOULD NEED TO GET THE ANNOUNCMENT DATE of all cities
> NOTE! Unfinished work here...

```{r}
# Restrict to treated that I have the announcment date:
main_dta <- main_dta %>%
  filter(!treated_uz | !is.na(date_announced))


start_scrappage = as.Date("14/01/2009", "%d/%m/%Y")
end_scrappage = as.Date("02/09/2009", "%d/%m/%Y")

main_dta <- main_dta %>%
  mutate(months_in_scrappage = end_scrappage - date_announced,
         # The maximum is the maximum days of program
         months_in_scrappage = ifelse(
           months_in_scrappage > (end_scrappage - start_scrappage),
           yes = (end_scrappage - start_scrappage),
           no = months_in_scrappage
         ),
         # The miimum is 0
         months_in_scrappage = ifelse(
           months_in_scrappage < 0,
           yes = 0,
           no = months_in_scrappage
         ),
         # For all years before 2009 there is no difference, all nuts had 0 months inthe scrappage program
         months_in_scrappage = ifelse(
           time < 2009,
           yes = 0,
           no = months_in_scrappage
         ),
         # For all control regions, they don't have the relative effect
         months_in_scrappage = ifelse(
           is.na(months_in_scrappage),
           yes = 0,
           no = months_in_scrappage
         )
  ) 
...
main_dta %>%
  select(geo, time,  date_announced, months_in_scrappage) %>%
  filter(!is.na(date_announced)) %>% unique()

# main_dta$months_in_scrappage %>% summary()
```
...
n3_dta <- main_dta %>%
  ...
  # )) %>%
  filter(!geo %in% c(  # not good synth C.
    # "DEA22", 
    "DEB35", 
    "DE713",
    "DEA55", # Not paralel trends
    # "DE714", 
    "DEA15", 
    "DEE02",
    "DE116",
...
> Magdeburg suffered from floods in 2013: Wikipedia +
> Was one of the most dynamic cities until 2011: https://www.welt.de/wirtschaft/article111642974/Magdeburg-ist-dynamischste-deutsche-Grossstadt.html
...
# 2011
filter((geo %in% c("DED41", "DE402", "DEF03")) | treated_uz) %>%   # Filter treated that have a confoundeing factor (Magdeburg)
  filter(geo != "DEE03")
