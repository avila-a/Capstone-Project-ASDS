...
dta_pollution_nuts <- pop_age_aggr %>% #Include NUTS 2 and NUTS 1 levels
  select(geo, time) %>%
...
```{r}
nuts_time <- pop_age_aggr %>% # NUTS 3 vars:
  left_join(gdp_complete) %>%
...
names(umweltzonen) <- c("State", "LEZ", "Status", 
