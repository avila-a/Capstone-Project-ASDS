...
not_optimal_treated <- c(
  "DE94", # Osnabrük has another big city in its NUTS 2 (A large zone)
...
out_n2 <- gsynth(wage_per_day ~ is_announced +  unempR_T  + gdp_eur_hab,
               data = n2_dta,
               index = c("geo","time"),
               na.rm = TRUE, # remove
               force = "two-way", # unit or time fixed effects or both?
...
dta_analysis <- dta_analysis %>%
  filter(FALSE == (treated_uz & year_stage1 <= 2009)) # or year_announced

dta_analysis %>%
  filter(treated_uz) %>%
  select(year_announced) %>%
  unique()
