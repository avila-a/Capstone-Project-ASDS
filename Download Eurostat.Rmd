...
* data has some "extra regions NUTS X{1,2,3}" in some countries and one for NUTS level. They account for extra regions (maritimal spaces, air space, and are optionals for countries to report) but have as third, fourth and 5th letter a Z instead of a number.
...
Compensation of employees (SAL, not EMP) (wages) by NACE activity	EU	NUTS 2
- 2000	2017	1 year	Eurostat	nama_10r_2coe 
- Million euros

_PROBLEMS WITH FRANCE_

```{r}
down_eurostat("nama_10r_2coe") -> Comp_emp_Nace_N2

Comp_emp_Nace_N2 <- Comp_emp_Nace_N2 %>%
  filter(currency == "MIO_EUR")

Comp_emp_Nace_N2$currency %>% table()
```
...
old_gdp <- read_delim("data/eurostat/refin.nama_10r_3gdp.dat",
                      "\t", 
                      trim_ws = TRUE,
                      col_types = cols(obs_decimals = col_skip(), 
                                       obs_status = col_skip())) 

old_gva <- read_delim("data/eurostat/refin.nama_10r_3gva.dat",
                      "\t", 
                      trim_ws = TRUE, # trim whitespace
                      col_types = cols(obs_decimals = col_skip(), 
                                       obs_status = col_skip()))

old_emp <- read_delim("data/eurostat/refin.nama_10r_3empers.dat",
                      "\t", 
                      trim_ws = TRUE, # trim whitespace
                      col_types = cols(obs_decimals = col_skip(), 
                                       obs_status = col_skip()))

# ! NOTE for this one france is still incomplete before 2008
old_wages <- read_delim("data/eurostat/refin.nama_10r_2coe.dat",
                      "\t", 
                      trim_ws = TRUE, # trim whitespace
                      col_types = cols(obs_decimals = col_skip(), 
                                       obs_status = col_skip()))

# # ! NOTE for this one is still incomplete before 2008, NOT WORTH IT
# # I did not continued with this one for now until I have better data
# old_hours2 <- read_delim("data/eurostat/refin.nama_10r_2emhrw.dat",
#                       "\t", 
#                       trim_ws = TRUE, # trim whitespace
#                       col_types = cols(obs_decimals = col_skip(), 
#                                        obs_status = col_skip()))
...
# ! NOTE for this one france is still incomplete before 2008
old_investment2 <- read_delim("data/eurostat/refin.nama_10r_2gfcf.dat",
                      "\t", 
                      trim_ws = TRUE, # trim whitespace
                      col_types = cols(obs_decimals = col_skip(), 
                                       obs_status = col_skip()))

old_investment2 %>% head()
...
old_hours2 <- old_hours2 %>%
  mutate(CNTR_CODE = substr(geo, 1, 2) %>% as.factor()) %>%
  select(-unit)

old_investment2 <- old_investment2 %>%
  mutate(CNTR_CODE = substr(geo, 1, 2) %>% as.factor()) %>%
  filter(currency == "MIO_EUR")  # Delete MIO_NAC entries
...
table((old_wages %>% dplyr::filter(CNTR_CODE == "PL"))$time) # %>% plot(type = 'l')
table((old_hours2 %>% dplyr::filter(CNTR_CODE == "FR"))$time) # %>% plot(type = 'l') # ! This is almost useless

# table((old_investment2 %>% dplyr::filter(CNTR_CODE == "NO"))$time) # %>% plot(type = 'l') # This is completely useless (NO and DE)
...
old_wages_new_codes <- dict_2013_2016_all %>%
  right_join(old_wages, by = c("Code 2013" = "geo")) %>%  #
  mutate(geo = ifelse(test = is.na(`Code 2016`), # have the "latest geo" code
                      yes = `Code 2013`, 
                      no = `Code 2016`)) 
...
Comp_emp_Nace_N2_complete <- old_wages_new_codes %>%
  #filter(time < 2015) %>% # Exclude 2015
  full_join(Comp_emp_Nace_N2) %>% # filter(geo == "FRK21", nace_r2 == "J") # %>% # New Ain (France, example)
  mutate(values = ifelse(test = is.na(values), # If there is no "new data"
                         yes = obs_value, # Take old data
                         no = values),
         CNTR_CODE = substr(geo, 1, 2)) %>%
  select(currency, nace_r2, time, geo, values, CNTR_CODE)
...
# This one continues to have missing data 200-2008 for FR and NO:
write.csv(Comp_emp_Nace_N2_complete, 
...
Economically active population by sex, age and NUTS 2 regions
- 1000s
- 1999	2019	1 year
- GE15 -> 15 years or over
