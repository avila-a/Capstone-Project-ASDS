...
Also inspired by the code for the replication files of Abadie et Al. (2013) 
> (REVIEW THIS!)
...
init_sc <- function(data = data, name_treat, outcome, mediator, 
  ...
  # print(paste0("Number of controls2: ", length(controls)))
  ...
  data_temp <- data_temp[order(data_temp[,col_id_num],
                               decreasing=F),]
  # remove(data2)
...
main_dta %>%
  ...
  filter(nuts_level == 3) %>%
  select(LEZ, geo, year_announced, year_stage1) %>% unique()
```
...
# countries_to_remove <- c("ES", "EL", "PT", # to avoid bias
#                          "RO", "LT") # to reduce complexity

countries_to_remove <- unique(main_dta$country)[unique(main_dta$country) != "DE"]
# ---

temp_1 <- main_dta %>% as.data.frame() %>%
    # If I use a variable that has only NUTS 2 regions
    filter(nuts_level == 3)  %>% 
    # Optional? restrict years ro 2001-2016, the period of mediator
    filter(time >= 2001 & time <= 2016)

temp <- balanced(data = temp_1 %>% filter(), 
                 TIME = "time", ID = "geo", 
                 VARS = c("mean_pol_gdp", "empl_TOTAL",
                          control_vars_labor, "contr_restr_60km"))

main_dta %>%
  select(geo, time, gva_G_J,
         empl_TOTAL, empl_TOTAL, `empl_G-J`) %>%
  filter(geo == "DEG01")
```
...
init_sc(data = main_dta,
        ...
        restr_contr_var = "contr_restr_60km",
        remove_countries = countries_to_remove) # True/false if it is valid control

# data %>%
#   filter(num_id %in% controls) %>%
#   group_by(num_id) %>%
#   summarise(num_id = first(num_id), 
#             urb_index = first(urb_index)) -> urb_index_d
# 
# urb_index_d %>%
#   filter(urb_index < 2.5) -> urb_index_d
# pretreat4
```
...
output_total <- do_synth_total(
  ...
  # Delete very rural areas from controls
  controls_identifier = controls[controls %in% urb_index_d$num_id],
...
ggplot() +
  ...
  labs(
    title = "Offenbach: Real and synthetic",
    y= "GDP/capita (€)",
    x = ""
  )
...
# temp <- balanced(data = temp_1 %>% filter(), 
#                  TIME = "time", ID = "geo", 
#                  VARS = c("mean_pol_gdp", "empl_rate_wpop",
#                           control_vars_labor, "contr_restr_60km"))
# 
# list_treated <- temp %>%
#   filter(treated_uz == TRUE) %>%
#   filter(nuts_level == 3,
#          year_stage1 <= 2016,
#          (!is.na(year_announced)), # has announcment date
#          # !(geo %in% c("DE929", "DE944"))
#          ) %>%
...
# Berlin is the ONLY nuts 3 region with this data complete :(
# Makes no sense to do 

for (treated in "DEG01"){
...
### Total Effect (before modifications)
...
### Try LOO with controls for a given LEZ (hannover):
...
output_total <- do_synth_total(
    ...
    dependent = Y,
    unit_variable = col_id_num,
    controls_identifier = controls, # full set of controls
    time_variable = col_time,
    treatment_id = name_treat,
    # treatment_id = data[data[,col_id]==name_treat & data[,col_time]==T_in,col_id_num],
    unit_names_variable = col_id,
...
# Get the control pool of our treated region
t_weights <- output_total$synth.tables$tab.w %>% as.data.frame() %>%
  arrange(desc(w.weights)) %>%
  filter(w.weights >= 0.003) ; t_weights

# SUPER IMPORTANT TO HAVE IT AS CHARACTER
control_units <- t_weights$unit.names %>% as.character()

# loop across control units (permutation test)
# I start with T_inf given it also saves the mediator
storegaps <- 
  matrix(NA,
        length(T_inf:T_fi2),
        length(control_units)
        )

rownames(storegaps) <- T_inf:T_fi2 
colnames(storegaps) <- control_units

# storegaps_M <- storegaps

i <- 1
# co <- unique(d$index)
for (k in control_units){
  
  print(paste0("init ", control_units[i]))
  
  # number_treated <- data[, 181][which("DEA4" %in%data[, 2])]
  
  init_sc(data = main_dta,
          nuts_level = 2,
          
          name_treat = control_units[i], # The control unit K
          t_fi = 2012, # the announcment date for Erfurt
  ...
  output_u_c <- do_synth_total(
    dta_imput = data,
    special_predictors = pretreat4,
    dependent = Y,
    unit_variable = col_id_num,
    
    #  All controls + treated - "control being treated"
    controls_identifier = controls,
      # c(controls_temp, "DE92")[-which(controls_temp==k)],
  ...
  storegaps[, i] <-  
    output_u_c$prepar$Y1-(output_u_c$prepar$Y0%*%output_u_c$synth.result$solution.w)
...
storegaps1 <- storegaps %>% as.data.frame %>% 
  tibble::rownames_to_column(var = "time") %>%
  mutate(time = as.numeric(time))

restult_treated <- output_total$prepar$Y1 - 
  (output_total$prepar$Y0 %*% output_total$synth.result$solution.w) %>% as_tibble()

storegaps1$Treated <- restult_treated$w.weight

storegaps_M <- storegaps1 %>%
  filter(time %in% T_inf:(T_in-1)) %>%
  # Correct time
  mutate(time = time + (T_in - T_inf))
  
storegaps_Y <- storegaps1 %>%
  filter(time %in% T_in:T_fi2)

deviation_gaps_M <- storegaps_M %>% 
  pivot_longer(names_to = "zone", cols = 2:12) %>%
  # Filter bad pre intervention fits (5x bigger than real one)
  filter(time < T_fi) %>%
  group_by(zone) %>%
  summarise(mean(value^2))

deviation_gaps_Y <- storegaps_Y %>% 
  pivot_longer(names_to = "zone", cols = 2:12) %>%
  # Filter bad pre intervention fits (5x bigger than real one)
  filter(time < T_fi) %>%
  group_by(zone) %>%
...
{
  deviation_treated_y <- (deviation_gaps_Y %>%
                          filter(zone == "Treated"))$`mean(value^2)`
  deviation_treated_y <- deviation_gaps_Y %>%
    filter(`mean(value^2)` < 10*deviation_treated_y)
  
  deviation_treated_m <- (deviation_gaps_M %>%
                          filter(zone == "Treated"))$`mean(value^2)`
  deviation_treated_m <- deviation_gaps_M %>%
    filter(`mean(value^2)` < 10*deviation_treated_m)
}

storegaps_M  %>% 
  pivot_longer(names_to = "zone", cols = 2:12) %>%
  filter(zone %in% deviation_treated_m$zone) %>%
  ggplot() +
  geom_path(aes(x= time, y = value, color = zone))

storegaps_Y %>% 
  pivot_longer(names_to = "zone", cols = 2:12) %>%
  filter(zone %in% deviation_treated_y$zone) %>%
  ggplot() +
  geom_path(aes(x= time, y = value, color = zone)) +
  geom_vline(xintercept = 2012) + # hamburg announcment
  labs(
    title = "Gaps in per capita GDP, Treated = Erfurt",
    y = "Per capita GDP in €/person",
    x = ""
  )
...
```{r}
post_pre_SQE <- storegaps_Y %>% 
  pivot_longer(names_to = "zone", cols = 2:12) %>%
  # Filter bad pre intervention fits (5x bigger than real one)
  mutate(pre_interv = ifelse(time < T_fi, "pre", "post")) %>%
  group_by(zone, pre_interv) %>%
  # calculate MSQE
  summarise(mean(value^2)) %>%
  pivot_wider(id_cols = zone, names_from = pre_interv,
              values_from = `mean(value^2)`) %>%
  # Do division
  mutate(post_pre_MSQE = post/pre) %>%
  # arrange(desc(post_pre_MSQE)) %>%
  mutate(zone = forcats::fct_reorder(zone, post_pre_MSQE))

ggplot(data = post_pre_SQE,
       aes(x= zone, y = post_pre_MSQE)) +
  geom_segment(aes(xend=zone, yend=0)) +
  geom_point() +
  coord_flip()
```
...
plot_treated <- data.frame(Y1 = output_total$Y1,
    ...
    geom_vline(xintercept = 
                 (main_dta %>%
                    filter(geo == name_treat))$year_stage1[1]) +
    # geom_vline(xintercept = NA) +
                 # (main_dta %>%
                 #    filter(geo == treated))$year_stage1[1]) +
    labs(
      title = paste0(name_treat, " ", (main_dta %>% filter(geo == name_treat))$year_announced[1])
...
t_weights <- output_total$synth.tables$tab.w %>% as.data.frame() %>%
  arrange(desc(w.weights)) %>%
  filter(w.weights >= 0.001) ; t_weights
...
for (k in control_units){
  ...
  init_sc(data = main_dta,
          ...
          name_treat = control_units[i], # The control unit K
          t_fi = 2007, # the announcment date for Hannover
...
storegaps1$DE92_T <- restult_treated$w.weight

storegaps_M_F <- storegaps1 %>%
  filter(time %in% T_inf:(T_in-1)) %>%
  # Correct time
  mutate(time = time + (T_in - T_inf))
  
storegaps_Y_F <- storegaps1 %>%
  filter(time %in% T_in:T_fi2)

deviation_gaps_Y <- storegaps_Y_F %>% 
  pivot_longer(names_to = "zone", 
               cols = 2:length(storegaps_Y_F)) %>%
  # Filter bad pre intervention fits (5x bigger than real one)
  filter(time < T_fi) %>%
  group_by(zone) %>%
  summarise(mean(value^2))
...
{
  deviation_treated <- (deviation_gaps_Y %>%
                          filter(zone == "DE92_T"))$`mean(value^2)`
  deviation_treated <- deviation_gaps_Y %>%
    filter(`mean(value^2)` < 20*deviation_treated)
}

storegaps_M_F  %>% 
  pivot_longer(names_to = "zone", cols = 2:10) %>%
  ggplot() +
  geom_path(aes(x= time, y = value, color = zone))

storegaps_Y_F %>% 
  pivot_longer(names_to = "zone", cols = 2:11) %>%
  filter(zone %in% deviation_treated$zone) %>%
  ggplot() +
  geom_path(aes(x= time, y = value, color = zone)) +
  geom_vline(xintercept = 2007) # hamburg announcment
