...
temp <- balanced(data = temp_1 %>% filter(), 
                 TIME = "time", ID = "geo", 
                 VARS = c("mean_pol_gdp", "gdp_eur_hab",
                          control_vars_labor, "contr_restr_60km"))

main_dta %>%
  select(geo, time, mean_pol,
         empl_TOTAL, gdp_eur_hab) %>%
  filter(geo == "DEG01")
```
...
storegaps <- storegaps %>% as.data.frame %>% 
  tibble::rownames_to_column(var = "time") %>%
  mutate(time = as.numeric(time))

storegaps$DE92_T <- output_total$prepar$Y1 - 
  (output_total$prepar$Y0 %*% output_total$synth.result$solution.w)
...

{
  ...
  deviation_treated <- deviation_gaps_Y %>%
    filter(`mean(value^2)` < 5*deviation_treated)
}
...
print(paste0("Synth things ", treated))
  ...
  print(output_unit_control$synth.tables$tab.pred %>%
          as.data.frame())
  print(output_unit_control$synth.tables$tab.w %>% 
          as.data.frame() %>%
          arrange(desc(w.weights)))
