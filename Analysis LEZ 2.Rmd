...
main_dta <- main_dta %>%
  # group_by(geo) %>%
  # mutate(
  #   year_inagurated = as_date(Stage_1, format = "%d.%m.%Y") %>% year(),
...
```{r}
out_n2 <- gsynth(unempR_T ~ is_inagurated,
                 # + wages_per_worker
                 # + gdp_eur_hab,
                 # urb_index, 
               data = n2_dta, 
               index = c("geo","time"), 
               na.rm = TRUE, # remove 
               force = "none", # unit or time fixed effects or both?
               # CV = TRUE, 
...
plot(out_n2, type = "counterfactual", id = "DE21") # 
