...
n3_dta <- n3_dta %>%
  # Not in the common suport
  filter(!(geo %in% c("DE711", "DE714"))) 
  # Magdeburg gas this very strong floods
...
### Work (Finantial, professional, Real State)
...
n3_dta <- main_dta %>%
  # restrict to NUTS with good synthetic controls
  filter(geo %in% c("DE711",
                    "DE713", "DEA5A") | treated_uz == FALSE) %>%
    # Remove all "treated" NUTS3 regions that are not representative of their cities
  filter( !(geo %in% not_optimal_treated)) %>%
  ...
  # filter(!(country %in% c("EL", "ES", "IT", "PT", "TR")))  %>% #,")))
  filter(!(geo %in% not_optimal_treated))
...
out_n2 <- gsynth(gva_rate_industry ~ is_announced_4y, 
               data = n3_dta, 
               index = c("geo","time"), 
               na.rm = TRUE, # remove NA
               force = "time", # unit or time fixed effects or both?
