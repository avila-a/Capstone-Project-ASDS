...
data <- data %>%
mutate(
  empl_prop_A = empl_A/empl_TOTAL,
  empl_prop_F = empl_F/empl_TOTAL,
  empl_prop_B_E = `empl_B-E`/empl_TOTAL,
  empl_prop_O_U = `empl_O-U`/empl_TOTAL,
  empl_prop_G_J = `empl_G-J`/empl_TOTAL,
  empl_prop_K_N = `empl_K-N`/empl_TOTAL,
)
...
```{r}
init_sc <- function(name_treat, outcome, mediator, 
                    id_col, id_col_num, col_time, delay = 0,
                    t_in, 
                    announced_or_implemented = c("A", "I"),
                    tfi2,
                    control_vars){
  # restrict the sample to a balanced panel
  { # delete any row with any missing values
    data_temp <- data[complete.cases(
      data %>%
        select(all_of(outcome), all_of(mediator), 
               all_of(id_col), all_of(col_time),
               all_of(control_vars)
        )), ]
    # delete geos that have a smaller number of rows
    incomplete <- which((table(data_temp[, id_col]) <
                           length(unique(data_temp[, col_time]))))
    # refresh dataset and create new set of numeric ids
    data_temp <- data_temp %>%
      dplyr::filter(!(id_col %in% names(incomplete))) %>% # Delete incomplete series
      dplyr::mutate(num_id = as.numeric(as.factor(geo))) %>%
      arrange(num_id)
  ...
  assign("name_treat", name_treat,
         envir = .GlobalEnv)
  assign("M", (str_detect(names(data), mediator) %>% which())[1], 
         envir = .GlobalEnv)
  assign("Y", str_detect(names(data), outcome) %>% which(), 
         envir = .GlobalEnv)
  assign("col_id", str_detect(names(data), id_col) %>% which(), 
         envir = .GlobalEnv)
  assign("col_id_num", str_detect(names(data), id_col_num) %>% which(), 
         envir = .GlobalEnv)
  assign("col_time", str_detect(names(data), col_time) %>% which(), 
         envir = .GlobalEnv)
  assign("T_in", t_in , envir = .GlobalEnv)
  if (announced_or_implemented[1] == "A"){
    assign("T_fi", data[data$geo == name_treat, "year_announced"][1], 
         envir = .GlobalEnv)
  } else {
    assign("T_fi", data[data$geo == name_treat, "year_stage1"][1], 
         envir = .GlobalEnv)
  }
...
# DEA2D -> Aachen, ¿tourist? city, implemented late 

# FAKE TREATED: DE60 Hambourg
...
init_sc(name_treat = "DE30",
        outcome = "hours_per_worker",
...
# name_treat <- "DE30" # name_treat<-"FRA"
# 
# # Mediator column
# M <- (str_detect(names(data), "mean_pol_gdp") %>% which())[1]
# # Outcome column
# Y <- str_detect(names(data), "hours_per_worker") %>% which()
# 
# 
# # Column with ID
...
# # NOTE: Change to "year_announced"
# 
# T_fi <- data[data$geo == name_treat, "year_announced"][1]
# # T_fi <- 2007
# 
...
#       select(
#         geo, time, 
#         unempR_T, mean_pol_gdp, # outcome and mediator
#         control_vars # control variables
#       )), ]
#   
#   incomplete <- which((data$geo %>% table(useNA = "no")) <
...
# Create a list of controls
{
  controls <- data %>%
    ...
    # filter(
    #   # geo != "LU00", # REMOVE LUXEMBOURG (ROBUSTNESS TEST)
    #   # geo != "DE60", # REMOVE HAMBOURG (ROBUSTNESS TEST)
    #   ) %>% 
    select(geo, num_id) %>% unique()
  controls_geo <- controls$geo
}
controls_geo
...
```{r}
controls_name <- controls_geo

# Keep only the ones that are controls or the treated unit
...
# take obervations that are NOT TREATED, and  in the time of 
controls <- data[!(data[,col_id]==name_treat) & data[,col_time]==T_in, col_id_num]


controls <- data[!(data[,col_id]==name_treat), col_id_num]
controls <- controls %>% unique
...
pretreat4[[length(pretreat4)+1]] <- list(M, T_fi, "mean")
...
prepar <- dataprep(data, 
                 ...
                 treatment.identifier = name_treat,
