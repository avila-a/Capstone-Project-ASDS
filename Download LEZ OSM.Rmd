...
for (i in 2:length(lat)){ # Does this include the ones who are cutted by the bbox?
  for (j in 2:length(long)){
    ...
    coordinates <- c(lat[i-1], long[j-1], lat[i], long[j])
    result_querry <- opq(
      bbox = coordinates, 
    ...
    final_geom <- bind_rows(final_geom, as.data.frame(result_geom))
...
lez_data <- final_geom %>%
  select(
    osm_id, name, boundary, note, source, start_date, website, geometry,
    access, osmwiki
  )
lez_data <- lez_data %>% st_sf()
lez_data %>% class
...
mapview(as(lez_data, 'Spatial'))

# Save the shape file!!

st_write(lez_data2, "lez_data2.shp")
methods(class = "sf")

```

## Try to read the saved shapefile (no success for now)

```{r}
lez_data2 <- readOGR(dsn = "lez_data2.shp")
lez_data2 %>% st_sf()
...
leaflet() %>% addTiles() %>%
  addPolygons(data = as(lez_data2, 'Spatial'), weight = 6, col = "red")
mapview(as(lez_data2, 'Spatial'))
```
