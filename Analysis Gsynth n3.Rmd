...
n3_dta <- main_dta %>%
  ...
  filter(geo %in% c(
    "DE143", "DEA1C", "DEE02", "DE724", "DEE03", "DE113", "DE115", "DE143") | treated_uz == FALSE) %>%
  ...
  #   filter(
  #   mean(gva_rate_retail, na.rm = TRUE) < 0.08,
  # ) %>%
  ungroup()
...
out_n2 <- gsynth(gva_rate_retail ~ is_announced_4y, #+ pop_km2 + pop_total,
               data = n3_dta, 
               index = c("geo","time"), 
               na.rm = TRUE, # remove NA
               force = "time", # unit or time fixed effects or both?
...
out_n2 <- gsynth(gva_rate_retail ~ is_announced,
               data = n3_dta, 
               index = c("geo","time"), 
               na.rm = TRUE, # remove NA
               force = "time", # unit or time fixed effects or both?
