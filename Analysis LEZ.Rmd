...
# Share of men/women in the labor market

## (for NUTS 3)

### Outcomes:
# Rate of employment (with active population or range of "active" years)
dta %>%
  mutate(empl_r_total = empl_TOTAL/pop_15_64)
...
```{r}
temp <- get_volatility(data = dta, CNTR_CODE = "DE",
                       time_vector = 2000:2019,
                       variable = "unempR_T")
dta %>% names()
...
temp$perc_change_dev %>% hist(breaks = 1000, xlim = c(-10,10))
temp$perc_change_dev %>% summary()
```
