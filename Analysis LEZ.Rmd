...
data_synth_c <- data_synth %>%
  filter(time < 2017, # Delete 2017 that has too many missing
         !is.na(gdp_eur_hab)) %>% # Delete those with NA
  group_by(geo) %>% # Delete those that do not have the whole period
  dplyr::mutate(na_in_geo = ifelse(n() == 17, FALSE, TRUE)) %>%
  filter(na_in_geo == FALSE) %>% select(-na_in_geo) %>%
  filter(!(appl_gz == 0 & appl_uar == 1)) # Delete suspicious controls
...
data %>% names()
panelView(gdp_eur_hab_norm ~ treat_gz, data = data_synth_c, 
...
out2 <- gsynth(gdp_eur_hab ~ treat_gz, 
              data = data_synth_c, 
              index = c("geo","time"), 
              force = "two-way", CV = TRUE, 
...
out2$eff.cnt %>% as.data.frame() %>% # Y - predicted Y(0);
  ...
  pivot_longer(-year, names_to = "geo", 
               values_to = "y_minus_y0") -> all_treat_effects
