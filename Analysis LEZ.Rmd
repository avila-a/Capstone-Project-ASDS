...
# png(filename = "images/LEZ application germany_2.png",
...
panelView(month ~ status, data = panel_zones, 
          ...
          legend.labs = c("No LEZ applied", "LEZ announced", 
                          "Red, yellow or green sticker", 
                          "Only yellow or green sticker", "Only green stiker"),
          color = c("grey90", "grey60", "red1", "yellow1", "green4")) +
  annotate("rect", xmin = 0, xmax=Inf, ymin=32.5, ymax=Inf,
                              alpha = 0.6, fill='white') +
  annotate("rect", xmin = 0, xmax=Inf, ymin=0.5, ymax=4.5,
                              alpha = 0.6, fill='white') +
  # geom_hline(yintercept = 26.5, size = 3, color = "white" ) +
  # geom_vline(xintercept = 33, size = 2) + #as.Date("01-09-2009", "%d-%m-%Y"))
  geom_segment(x = 33, xend = 33, y = 4.5, yend = 32.5, size = 1.5) +
  geom_segment(x = 12*10, xend = 12*10, y = 4.5, yend = 32.5, size = 1.5) 
  # geom_vline(xintercept = 12*10, size = 2) 
# dev.off()
