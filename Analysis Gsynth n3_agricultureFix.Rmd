...
n3_dta <- main_dta %>%
  ...
  filter( # Restrict difference in agriculure weight to reduce interpolation biases
    mean(gva_rate_agriculture, na.rm = TRUE) < 1/100,
  ) %>%
  ungroup()
...
dta_for_summary <- n3_dta %>%
  select(geo, gdp_eur_hab, pop_km2, pop_total, treated_gz,
         gva_rate_local_trade, gva_rate_agriculture,
         gva_rate_public, gva_rate_work,
         gva_rate_construction, gva_rate_industry) %>%
  group_by(geo) %>%
...
vars <- dta_for_summary[
  ,c('gdp_eur_hab', 'pop_km2', 
     "gva_rate_industry",
     "gva_rate_construction",
     "gva_rate_local_trade",
     "gva_rate_agriculture",
     "gva_rate_public",
     "gva_rate_work")]
group <- dta_for_summary[,c('treated_summ')]

# Turn rate into percentages
get_percentage <- function(x){x*100}

vars <- vars %>%
  mutate_at(vars(matches("gva")), get_percentage)
...
for (i in fake_tr_dta[!is.na(fake_tr_dta$year_announced), ]$geo %>% unique()){
  if (i %in% out_n2$id.tr){
    placebo_plot <- plot(
      out_n2, type = "counterfactual", id = i,
      xlim = c(2000, fake_tr_dta[fake_tr_dta$geo == i,
                                 ]$year_announced[1]-1),
      # main = "",
      xlab = "Year",
    ...
    pdf(file=paste0("images/prelim_results/plac2011/ind_inverse_placebo",
                    "_",  i, ".pdf"),
        width = 3.5, height = 3.1)
    plot(final_plot)
    dev.off()
...
ATT_plot <- out_n2$est.att %>%
  as_tibble(rownames = "rel_time") %>%
  mutate(
    # This puts 2010 as the year "0"
    rel_time = as.numeric(rel_time)-10+2010,
    # This gives the real ATC as the "treated" are not really treated
    ATT = -ATT, CI.lower = -CI.lower, CI.upper = -CI.upper) %>%
  ggplot(aes(x= rel_time, y = ATT, group = 1)) +
  geom_ribbon(aes(ymin = CI.lower, ymax = CI.upper),
  ...
  geom_segment(x = 2011, xend = 2011, y = -5000, yend = 1000, 
               size = 2, color = "grey50") +
  geom_line(size = 1.5) +
  geom_text(aes(x= rel_time, y = 1500, label = n.Treated)) +
  geom_hline(yintercept = 0) +
...
{
pdf(file=
      paste0("images/prelim_results/plac2011/ATT_inv_real.pdf"),
    width = 5.5, height = 3)
print(ATT_plot)
dev.off()
}
...
for (i in fake_tr_dta[!is.na(fake_tr_dta$year_announced), ]$geo %>%
  ...
  if (i %in% out_n2$id.tr){
    ...
    # pdf(file=paste0("images/prelim_results/plac2011/ind_inverse_real",
    #                 "_",  i, ".pdf"),
    # width = 3.5, height = 3.1)
    print(temp)
    # dev.off()
...
dta_for_summary <- fake_tr_dta_real %>%
  select(geo, gdp_eur_hab, pop_km2, pop_total, treated_gz,
         gva_rate_local_trade) %>%
  group_by(geo) %>%
  summarise_all(function(x){mean(x, na.rm = TRUE)}) %>%
...
result <- tableContinuous(vars = vars, group = group, 
                stats = c("n", "min", "mean", "max", "s"),
                prec = 0,font.size = "small",
                # print.pval = "anova",
                cap = "Summary statistics for potential effect on GDP/capita",
                lab = "tab: descr stat inv2012",
                longtable = FALSE
                # file = "Presentations and documents/results/des_gdpcapita.tex"
                )
...
weights_treated_pool <- out_n2$wgt.implied %>%
  ...
  left_join(names_geo, by = c("control" = "geo")) %>%
  rename(name_control = nuts_name) %>%
  mutate(name_control = stringr::str_remove(name_control,
                                             ",.*$")) %>%
  select(-simple_LEZ)
...
ggplot(data = weights_treated_pool) +
  geom_point(aes(x = name_control, y = value, 
                 color = name_treated, shape = name_treated),
             size = 3) +
  coord_flip() +
  labs(
    y = "Implied Weight",
  ...
  # # scale_colour_discrete("", name = "Treated Po") +
  scale_shape_manual("Treated Regions", values=c(15,16,17,9)) +
  labs(color  = "Treated Regions", shape = "Treated Regions")
# dev.off()
...
for (i in fake_tr_dta_real[fake_tr_dta_real$treated_uz == FALSE, ]$geo %>% unique()){
  if (i %in% out_n2$id.tr){
    ...
    # pdf(file=paste0("images/prelim_results/plac2011/ind_inverse_real_eff",
    #                 "_",  i, ".pdf"),
    # width = 4, height = 3.1)
    print(plot_i)
    # dev.off()
...
#   addPolygons(data = donor_pool, weight = 0.5, color = "black",
#               fillColor = ~colorQuantile("RdYlGn",
#                       donor_pool$DEF03)(donor_pool$DEF03),
#               popup = paste0(
#                 "<b>", donor_pool$NUTS_ID, "</b> <br/>",
#                 "<b> Weight Berlin </b>", donor_pool$DEF03, "<br/>",
#                 "<b> Weight Hannover </b>", donor_pool$DEF03, "<br/>")
#               )
...
factor_weights <- nuts_treated %>%
  right_join(factor_weights) %>%
  select(NUTS_ID, r1, treatment) %>%
  rename(c("Factor 1" = r1)) #, "Factor 3" = r3))
...
temp <- factor_weights %>%
  left_join(main_dta %>% 
            ...
            by = c("NUTS_ID" = "geo")) %>%
  unique() %>%
  mutate(name_control = nuts_name) %>%
  mutate(name_control = stringr::str_remove(name_control,
                                             ",.*$")) %>%
  arrange(desc(`Factor 1`)) %>%
  mutate(name_control = forcats::fct_inorder(name_control))


# # Factor 1, Rural - VS - Urban "economy" (or more gdp_capita) 

tm_map_w <- tm_shape(germany) +
  tm_polygons() +
tm_shape(factor_weights %>% filter(treatment == "Treated")) +
  tm_dots(size = 2, alpha = 0.2) +
tm_shape(factor_weights) +
  tm_polygons(col = "Factor 1",
              style = "cont") +
tm_shape(temp) +
  tm_text("name_control",
          size = 0.7, shadow = TRUE, 
          # auto.placement = 1.21,
          just = "right",xmod = -0.5) +
  tm_layout(
...
tm_facet_map <-  tm_shape(temp) +
  tm_polygons(col = "Factor 1",
              style = "cont", border.col = NA) +
  tm_facets(by="name_control") +
tm_shape(factor_weights %>% 
             filter(treatment == "Treated")) +
  # tm_dots(size = 50, alpha = 0.2) +
...
{
# pdf(file=paste0("images/prelim_results/plac2011/w_facet.pdf"),
#     width = 5, height = 3.1)
tm_facet_map
# dev.off()
}
{
# pdf(file=paste0("images/prelim_results/plac2011/w_map.pdf"),
#     width = 4, height = 3)
tm_map_w
# dev.off()
}
...
{
# pdf(file=paste0("images/prelim_results/plac2011/w_map_2.pdf"),
#     width = 4, height = 3)
tm_map_w
# dev.off()
}
...
Factor.labs <- c("Factor 1", "Factor 2",
                          "Factor 3", "Factor 4")
names(Factor.labs) <- c("r1", "r2", "r3", "r4")

weights_ggplot <- weights %>%
  left_join(geos_num) %>%
  pivot_longer(cols = r1:r4, names_to = "Factor") %>%
  mutate(Factor = forcats::fct_recode(Factor, "Factor 1" = "r1",
                           "Factor 2" = "r2",
                           "Factor 3" = "r3",
                           "Factor 4" = "r4"
                           ))

weights_hist <- ggplot(weights_ggplot) +
  geom_histogram(aes(x = value, fill = Group)) +
  ggrepel::geom_text_repel(data = weights_ggplot %>%
                             filter(value >2),
                           aes(x = value, y = 1,
                                label = LEZ), 
                           angle = 90, 
...
plot_factor_paths <- out_n2$factor %>%
  ...
  pivot_longer(cols = r1:r4, 
               values_to = "value", names_to = "factor") %>%
  mutate(factor = forcats::fct_recode(factor, "1" = "r1",
                           "2" = "r2",
                           "3" = "r3",
                           "4" = "r4"
                           )) %>%
...
pdf(file="images/prelim_results/trade/f_loadings.pdf",
    width = 4.5, height = 2.7)
weights_hist
dev.off()

pdf(file="images/prelim_results/trade/factors_time.pdf",
    width = 3, height = 3)
plot_factor_paths
dev.off()
  ...
  if (i %in% out_n2$id.tr){
    ...
    plot_i <- out_n2$est.ind[, ,i] %>%
      ...
      labs(
        title = paste0(i, " - " ,n3_dta[n3_dta$geo == i,]$LEZ[1]),
        x = "Year",
        y = "Differences in Trade' share of GVA"
      ) +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1))
    print(plot_i)
    # dev.off()
...
factor_weights <- nuts_treated %>%
  right_join(factor_weights) %>%
  select(NUTS_ID, treatment, r1, r2, r3, r4) %>%
  rename(c("Factor 1" = r1, "Factor 2" = r2, 
           "Factor 3" = r3, "Factor 4" = r4))



# # Factor 1, Rural - VS - Urban "economy" (or more gdp_capita) 
# 
# pdf(file="images/prelim_results/trade/factor_1_map.pdf",
#     width = 2.8, height = 3)
tm_shape(germany) +
  tm_polygons() +
tm_shape(factor_weights) +
...
# pdf(file="images/prelim_results/trade/factor_1_propGVA.pdf",
#     width = 3, height = 3)
factor_weights %>%
...
# pdf(file="images/prelim_results/trade/factor_2_map.pdf",
#     width = 2.8, height = 3)
tm_shape(germany) +
  tm_polygons() +
tm_shape(factor_weights) +
...
# pdf(file="images/prelim_results/trade/factor_3_map.pdf",
#     width = 2.8, height = 3)
tm_shape(germany) +
  tm_polygons() +
tm_shape(factor_weights) +
...
# pdf(file="images/prelim_results/trade/factor_4_map.pdf",
#     width = 2.8, height = 3)
tm_shape(germany) +
  tm_polygons() +
tm_shape(factor_weights) +
  tm_polygons(col = "Factor 4") +
tm_shape(factor_weights %>% filter(treatment == "Treated")) +
  tm_borders(col = "black") +
  tm_layout(legend.bg.color = "white", 
            legend.position = c("right", "bottom"),
